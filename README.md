## `Mixture model` means a linear combination of responses while `Hierachicial (multi-level) model` means the nesting distribution of parameters. The `Mixture` is used to reflect the presence of subpopulations within an overall population (but no need to identify the sub-population to which an individual observation belongs) and the `Hierarchical` is used otherwise. You cannot use information criteria - WAIC, etc. - for model comparison in the `Mixture` while the `Hierarchical` can use them because it assigns heterogeneity in probabilities (at each record) at any level of aggregation.    
